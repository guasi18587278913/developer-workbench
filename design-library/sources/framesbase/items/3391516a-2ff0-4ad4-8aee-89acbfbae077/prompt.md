# PROMPT: Recreate "TOKYO NOCTURNE" — scroll-scrubbed cinematic one-pager

Build a single self-contained `index.html` (everything inline: CSS + JS in one file, no build step). It is a black, film-graded, scroll-driven one-page site for a fictional Tokyo night-riding crew. The page has NO scrolling content in the normal sense — every visible layer is `position: fixed`, and three invisible spacer divs ("runways") create scroll height. One scroll listener + one rAF loop convert `scrollY` into CSS custom properties and a video scrub.
...
- Title: `TOKYO NOCTURNE · Shutoko C1 night runs`
- Meta description: `A night-ride crew running sport bikes on the Shutoko C1 loop through central Tokyo.`
...
- **HERO (scrubbed by scroll, never played):**
  `https://d8j0ntlcm91z4.cloudfront.net/user_3GJaYKPxdnQG0Q9O26lu6DPmcHu/hf_20260806_233903_cc2b0f36-8843-4e18-a751-22171634be40.mp4`
  ...
  This clip: 97 frames, ~4.04s, 24fps, a single keyframe at t=0 (that single-keyframe GOP is why the frame bank exists).
- **REVEAL (plays + loops when the split opens, never scrubbed):**
  `https://d8j0ntlcm91z4.cloudfront.net/user_3GJaYKPxdnQG0Q9O26lu6DPmcHu/hf_20260806_181659_f9fcad4b-f561-45fc-b9d5-ba40a0627b34.mp4`
  Attributes: `class="reveal__video" data-reveal muted loop playsinline preload="auto"`. No poster.
- Poster `hero-2.jpg` is a local still from the same shoot.
...
1. **`.stage`** (z 0): `position:fixed; inset:0; background:#000; overflow:hidden; opacity: calc(1 - var(--stage-out));`
   ...
   - `.stage::after`: flat wash `background: rgba(0,0,0,.30)` covering the stage (a mood knob, not a measured scrim; type blends with `difference` over raw footage and is allowed to wash out on bright frames).
2. **`.reveal`** (z 5): the diagonal split (see §5).
3. **`.grain`** (z 6): film grain over BOTH videos, under all type (see §4).
4. **`.sec2`** and **`.sec3`** (z 8): fixed text layers with `mix-blend-mode: difference`.
5. **`.frame`** (z 10): the hero's typographic layer, also `mix-blend-mode: difference`.

CRITICAL blend rule: `mix-blend-mode: difference` MUST sit on the fixed layers (`.frame`, `.sec2`, `.sec3`) themselves, never on text children — a fixed element forms its own stacking context, so a blend on a child would resolve against empty transparency and do nothing.

---

## 4. FILM GRAIN (no network dependency)

`.grain`: `position:fixed; top:-50vh; left:-50vw; width:200vw; height:200vh; z-index:6; pointer-events:none; opacity: var(--noise-opacity);`
Background is an inline SVG data-URI, 200×200, `background-repeat:repeat; background-size:200px 200px`:
`feTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4' stitchTiles='stitch'` → `feColorMatrix type='saturate' values='0'` (raw turbulence is coloured; desaturate or it reads as a bug) → `feComponentTransfer` with linear R/G/B `slope='3.5' intercept='-1.48'` and A `slope='0' intercept='1'`.
Animation: `animation: grain 500ms steps(1) infinite;` — 10 held `transform: translate(x%, y%)` offsets per cycle (= 20Hz snaps): `0%,100%: (0,0); 10%: (-4%,-6%); 20%: (-8%,3%); 30%: (4%,-8%); 40%: (-6%,6%); 50%: (-9%,-3%); 60%: (6%,4%); 70%: (-3%,8%); 80%: (7%,-5%); 90%: (-4%,7%)`. `steps(1)` snapping IS the film-grain read; transform-only so it runs on the compositor.

---

## 5. THE REVEAL (diagonal split → section-3 card)
...
## 6. RUNWAYS (only in-flow elements)
...
<h1 class="brand">
  <span class="brand__slot">
    <span class="brand__line" data-role="main">Tokyo</span>
  ...
  <span class="brand__slot">
    <span class="brand__line" data-role="main">Nocturne</span>
    <span class="brand__line" data-role="alt" aria-hidden="true">The&nbsp;Loop</span>
    <span class="brand__line" data-role="crew" aria-hidden="true">Of&nbsp;Twelve</span>
  </span>
  <span class="brand__jp" lang="ja" aria-hidden="true">
    <span>トウキョウ</span><span>・</span><span>ノクターン</span>
  </span>
</h1>
```
...
- `.brand__slot`: `display:grid; overflow:hidden; margin-left:-0.02em;` (cancels T/N side bearing). All three versions of a line occupy the SAME grid cell (`grid-area: 1/1` on `.brand__line`), so the slot takes the widest line's width and nothing reflows on swap; overflow clips the travelling line. First slot: `margin-top:-0.13em` (trims Inter Tight's ascent dead space so the cap meets the frame edge).
...
- `.brand__char[data-jp]`: `font-family:'Noto Sans JP','Hiragino Sans','Yu Gothic',sans-serif; font-weight:700; font-size:0.92em; letter-spacing:0; transform: scaleX(0.813);` (0.92em matches ink height 74.71/81.10; 0.813 condenses kana advance 92.1→74.9 to sit in the locked width).
...
- Clock: `<time class="clock__time" id="clock" aria-live="off">[ 02:41 AM ]</time>` + `<span class="clock__zone">Japan<br>Standard Time</span>` (`.clock__zone`: `margin-top:3px; font-size:var(--fs-nano); line-height:1.15; opacity:.92;`). `.clock__time { font-variant-numeric: tabular-nums; }`
- Nav (`aria-label="Primary"`, gap `clamp(14px,1.5vw,26px)`): links `Routes / Machines / Film / Crew` (hrefs `#routes` etc.), each `class="hit"`.
- Contact link: `↳&nbsp;Ride with us` (`&#8627;`), href `#contact`, class `contact micro hit`.
...
Copy: *"We ride the C1 loop after the last train clears Shibuya. Four laps. Wet elevated concrete, the tunnels to ourselves, and nobody up here until the delivery trucks come off the Kasumigaseki ramp."*
...
## 8. SECTION 2 (fixed layer, z 8)

```html
<section class="sec2" aria-label="The run in numbers">
  <h2 class="s2-statement">Four laps of the C1. From last train to first truck.</h2>
</section>
```
...
## 9. SECTION 3 (fixed layer, z 8)

```html
<section class="sec3" aria-label="The crew">
  <p class="s3-left"  style="--i:0">Twelve of us. Four have ridden the loop since 2019, the rest turned up one at a time and stayed.</p>
  <p class="s3-right" style="--i:1">No patches and no club. Two bikes minimum, five maximum, and nobody rides ahead of whoever knows the exits.</p>
  <p class="s3-bottom">Twelve of us<br>Nine bikes<br>One route</p>
</section>
```
...
- `@media (max-width: 767px)`:
  ...
  `.frame` → 1 column, rows `auto auto 1fr auto`; chrome row 1, brand row 2 (`margin-top:5vh`), lede row 3 (`margin-bottom:7vh; max-width:42ch`), foot row 4. `.stage__media { object-position: 66% 50%; }` (bias to the rider). `.sec2` → 1 column, statement `grid-area: 2/1`. `.sec3` rows → `auto 1fr auto auto` with `column-gap: var(--frame)`; `.s3-bottom` row 3; notes drop to row 4 side-by-side (`align-self:end; max-width:none; margin-top:1.2em`).
...
- `@media (prefers-reduced-motion: reduce)`: kill nav/contact transitions, grain animation, brand-line transitions (title still swaps, without travel); `.s2-word { filter:none; transform:none; }` (opacity-gating only). In JS: no scrub, no frame bank, no reveal playback, no GSAP intro.

---

## 11. JAVASCRIPT

### 11a. Live Tokyo clock (IIFE, runs at parse time)
`Intl.DateTimeFormat('en-US', { timeZone:'Asia/Tokyo', hour:'2-digit', minute:'2-digit', hour12:true })` → paints `[ HH:MM AM ]` into `#clock` plus a `datetime` ISO attribute. First tick aligned to the minute boundary via `setTimeout(60000 - Date.now() % 60000)`, then `setInterval` 60s.

### 11b. Scroll narrative (main IIFE)
...
**KANA map (fixed partner per letter):** `{ T:'タ', O:'オ', K:'カ', Y:'ヤ', N:'ナ', C:'ク', U:'ウ', R:'ラ', E:'エ' }`
...
- `solveTitleTrigger()`: walk `r` 0.01→1 step 0.005; the title corner (at its SHRUNK position) is inside the rotated strip when `|px·cos(θ) + py·sin(θ)| ≤ r·span/2` with `θ = r·45°`; map that `r` into progress via `CONFIG.reveal`. Fallback 0.95. (Lands ≈0.913 at 1440×900.)
...
**Scrub loop (rAF):** two paths, one clock. `smoothed += (target - smoothed) * lerp` with epsilon snap. If the frame bank is live → `bank.draw(smoothed)`. Else if the whole clip is buffered (`buffered().end ≥ duration - 0.1`) AND `!hero.seeking` and delta > epsilon → `hero.currentTime = smoothed` (the `!seeking` guard is what keeps the single-keyframe seek storm survivable). On `loadeddata`: seed `smoothed = scrubTime(0)` and seek the hero there (don't creep from 0).

### 11c. FRAME BANK (WebCodecs, best-effort, the "smooth path")
Problem: the clip has ONE keyframe at t=0, so `currentTime = x` decodes everything from zero (60–260ms) → 4–15fps scrub. Solution: pay the GOP once up front.
- Skip entirely if reduced motion or no `window.VideoDecoder`. Get canvas 2d ctx with `{alpha:false}`. Set `hero.preload='metadata'` while building (don't download 21MB twice); on ANY failure `revert()`: restore `preload='auto'`, `hero.load()`, log `[frame bank] <why> — staying on the video scrub` via console.info. A `reverted` latch prevents a late `finish()` lighting the canvas after the watchdog fired.
- **60-second watchdog** (measured: fetch alone took 19.1s on a slow connection; 15s killed working builds).
...
- Decoder output: for each VideoFrame, `createImageBitmap` → draw to an offscreen canvas → `toBlob('image/webp', 0.82)` (~150–250KB stills; 97 live GPU frames would be hundreds of MB and get dropped). Track `encodeQueue`; `finish()` only when flushed && queue empty && every slot has a blob. **Sort by presentation timestamp** before banking (samples arrive in decode order with B-frame cts deltas; the decoder's contract emits presentation order, but the sort is cheap insurance). `fps = count / hero.duration`; set `live = true`, `canvas.setAttribute('data-live','')`, prewarm frame 0.
...
### 11d. GSAP intro (DOMContentLoaded)
...
## 12. NARRATIVE ARC (what the user experiences)

1. **Load:** black → cinematic cascade (video blurs in, title lines rise, katakana staggers, chrome, lede, footer).
2. **Scroll (hero, 350vh):** the video scrubs frame-locked to scroll (canvas path at display rate once the bank builds). The wordmark shrinks into the corner (to exactly 120px tall, max 0.51×); single letters flicker to katakana one at a time on the way; the katakana row dissolves early (blur+lift), lede+footer dissolve past halfway.
3. **The split (last 80vh):** a diagonal strip opens from the centre — rotating to 45° as it widens — revealing the second looping clip, picture upright. When the strip's edge geometrically reaches the shrunk title, both lines roll vertically: TOKYO/NOCTURNE → ENTER/THE LOOP, with a 3-flip kana burst. The hero plate fades inside the final corner sliver (`--reveal` 0.93→0.98).
4. **Section 2 (120vh):** "FOUR LAPS OF THE C1. FROM LAST TRAIN TO FIRST TRUCK." types itself in word-by-word (bottom-right, exactly the shrunk-title size), over the still-looping reveal clip.
5. **Section 3 (220vh):** the statement cascades out word-by-word; the fullscreen clip unwinds its rotation and shrinks to a centred card (0.26×) while its black frame fades; the heading rolls a third time → CREW/OF TWELVE (with burst) exactly as the card's edge clears it; two margin notes fade in flanking the card (staggered 0.62/0.72); the closing line "TWELVE OF US / NINE BIKES / ONE ROUTE" (hand-set breaks preserved) types itself in centred at the bottom.
